...
export type Event = {
  ...
  location?: {
    ...
    city?: string
    state?: string
    country?: 'USA' | 'Canada' | 'UK' | 'Switzerland' | 'Italy'
  }
  eventRecordings?: Array<
    {
      _key: string
    } & Recording
  >
...
// Query: *[_type == "presentation" && defined(slug.current) && defined(pubDate)]		| order(pubDate desc)[0...20] {				_id,	_type,	"slug": slug.current,				"genre": genre->prefLabel,	"topics": topic[]->prefLabel,			pubDate,			title,			description,			bodyText,			highlights		}
...
// Source: ../web-next/src/sanity/queries/home.ts
// Variable: GENRE_TREE_QUERY
// Query: {		"topConcepts": *[_type == "skosConceptScheme" && title == "Genre"][0].topConcepts[]->{			_id,			prefLabel		},		"concepts": *[_type == "skosConcept"]{			_id,			prefLabel,			"broader": broader[]._ref		}	}
export type GENRE_TREE_QUERY_RESULT = {
  topConcepts: Array<{
    _id: string
  ...
  concepts: Array<{
    _id: string
    prefLabel: string | null
    broader: Array<string> | null
  }>
}

// Source: ../web-next/src/sanity/queries/home.ts
// Variable: HOME_AUTHOR_QUERY
// Query: *[_type == "settings"][0]{		authorName,		"authorImage": authorImage{asset, crop, hotspot, altText}	}
export type HOME_AUTHOR_QUERY_RESULT = {
...
// Source: ../web-next/src/sanity/queries/home.ts
// Variable: HOME_TAXONOMY_QUERY
// Query: {		"genres": *[_type == "skosConcept" && _id in $documentGenres]{			prefLabel,			"count": count(*[				_type in ["article", "caseStudy", "note"] && genre._ref == ^._id			])		},		"topics": *[			_type == "skosConcept"			&& count(*[_type in ["article", "caseStudy", "note"] && ^._id in topic[]._ref]) > 0		]{			prefLabel,			"count": count(*[				_type in ["article", "caseStudy", "note"] && ^._id in topic[]._ref			])		}	}
export type HOME_TAXONOMY_QUERY_RESULT = {
  genres: Array<{
    prefLabel: string | null
    count: number
  }>
...
// Source: ../web-next/src/sanity/queries/home.ts
// Variable: HOME_INSIGHTS_QUERY
// Query: {		"articles": *[			_type in ["article", "caseStudy", "note"]			&& defined(slug.current)			&& genre._ref in $articleGenres		] | order(pubDate desc)[0...$articleCount] {				_id,	_type,	"slug": slug.current,				pubDate,	_updatedAt,				"genre": genre->prefLabel,	"topics": topic[]->prefLabel,			title,			shortDescription,			heroImage { 	asset,	crop,	hotspot,	altText,	caption }		},		"notes": *[			_type in ["article", "caseStudy", "note"]			&& defined(slug.current)			&& genre._ref in $noteGenres		] | order(pubDate desc)[0...$noteCount] {				_id,	_type,	"slug": slug.current,				pubDate,	_updatedAt,				"genre": genre->prefLabel,	"topics": topic[]->prefLabel,			title,			shortDescription,			clipRef { publisher }		}	}
export type HOME_INSIGHTS_QUERY_RESULT = {
  articles: Array<
    | {
        _id: string
        _type: 'article'
...
// Query: *[_type in ["article", "caseStudy", "note"] && slug.current == $slug][0] {			_id,	_type,	"slug": slug.current,			pubDate,	_updatedAt,			"genre": genre->prefLabel,	"topics": topic[]->prefLabel,		title,		shortDescription,		description,		lede,		bodyText,		heroImage { 	asset,	crop,	hotspot,	altText,	caption },		clipRef {			clipUrl,			publisher,			title,			img { asset, crop, hotspot, altText }		},		bookRef {			bookUrl,			title,			author,			publisher,			pubDate,			img { asset, crop, hotspot, altText }		}	}
...
// Query: *[_type in ["article", "caseStudy", "note"] && defined(slug.current)] {		"params": {"slug": slug.current}	}
...
// Source: ../web-next/src/sanity/queries/pages.ts
// Variable: PAGE_WORK_WITH_ME_BAND_QUERY
// Query: *[_type == "page" && slug.current == $slug][0] {			"workBand": select(		"bandWorkWithMe" in coalesce(pageBands, []) => coalesce(			customBands[_type == "bandWorkWithMe"][0],			*[_type == "settings"][0].defaultBands[_type == "bandWorkWithMe"][0]		)	){		message,		"clientLogos": clientLogos[]->{			name,			"image": tile{asset, crop, hotspot, altText},			"caseStudy": *[_type == "caseStudy" && client._ref == ^._id]				| order(pubDate desc)[0].slug.current		}	}	}
export type PAGE_WORK_WITH_ME_BAND_QUERY_RESULT = {
  workBand: {
    message: Array<{
      children?: Array<{
        marks?: Array<string>
...
// Source: ../web-next/src/sanity/queries/presentations.ts
// Variable: PRESENTATION_DETAIL_QUERY
// Query: *[_type == "presentation" && slug.current == $slug][0] {			_id,	_type,	"slug": slug.current,			pubDate,	_updatedAt,			"genre": genre->prefLabel,	"topics": topic[]->prefLabel,		title,		description,		poster { 	asset,	crop,	hotspot,	altText,	caption },		bodyText,		highlights	}
export type PRESENTATION_DETAIL_QUERY_RESULT = {
  _id: string
  _type: 'presentation'
  slug: string | null
  pubDate: string | null
...
// Source: ../web-next/src/sanity/queries/presentations.ts
// Variable: PRESENTATION_DELIVERY_QUERY
// Query: *[_type == "presentation" && slug.current == $slug][0] {		"deck": presentationDeck.asset->{url, originalFilename, size, extension},		"events": eventDetail[]->{			_id,			"name": event,			date,			link,			"online": location.online,			"city": location.city,			"state": location.state,			"country": location.country,			"recordings": eventRecordings[]{				kind,				url,				mediaUrl,				sourceName,				duration,				poster { 	asset,	crop,	hotspot,	altText,	caption }			}		}	}
export type PRESENTATION_DELIVERY_QUERY_RESULT = {
  deck: {
    url: string | null
...
// Source: ../web-next/src/sanity/queries/presentations.ts
// Variable: PRESENTATION_RSS_BAND_QUERY
// Query: *[_type == "presentation" && slug.current == $slug][0] {			"rssBand": coalesce(		customBands[_type == "bandRss"][0],		*[_type == "settings"][0].bandOverrides[documentType == ^._type][0].overrideBands[_type == "bandRss"][0],		*[_type == "settings"][0].defaultBands[_type == "bandRss"][0]	){title, message, buttonTarget}	}
export type PRESENTATION_RSS_BAND_QUERY_RESULT = {
  rssBand: {
    title: string | null
    message: Array<{
      children?: Array<{
...
// Query: {		"genres": *[_type == "skosConceptScheme" && title == "Genre"][0] {			"tops": topConcepts[]->{_id, prefLabel},			"concepts": concepts[]->{				_id,				prefLabel,				"parent": broader[0]._ref			}		},		"pool": *[			_type in ["article", "caseStudy", "note", "presentation"]			&& defined(slug.current)			&& defined(pubDate)		] | order(pubDate desc) {				_id,	_type,	"slug": slug.current,				pubDate,	_updatedAt,			title,			shortDescription,			"genreId": genre._ref,			"genre": genre->prefLabel,			heroImage { 	asset,	crop,	hotspot,	altText,	caption },			"sourceDomain": clipRef.publisher,			"topics": topic[]->{				_id,				"parent": broader[0]._ref,				"parentIsTop": !defined(broader[0]->broader[0]._ref)			},			"poster": coalesce(				poster,				(eventDetail[]->eventRecordings[])[defined(poster)][0].poster			) { 	asset,	crop,	hotspot,	altText,	caption },			"venue": eventDetail[0]->{				"name": event,				"online": location.online,				"city": location.city,				"state": location.state,				"country": location.country			},			"eventCount": count(eventDetail),			"hasTranscript": defined(transcript),			"hasDeck": defined(presentationDeck.asset),			"recordingKinds": (eventDetail[]->eventRecordings[])[defined(kind)].kind		}	}
...
// Source: ../web-next/src/sanity/queries/reviews.ts
// Variable: REVIEWS_BY_CLIENT_QUERY
// Query: *[_type == "client" && count(*[_type == "review" && references(^._id)]) > 0] {		_id,		name,		relationship,		role,		"logo": logo{asset, crop, hotspot, altText},		"engagements": engagementDates[]{startDate, endDate},		"latestEnd": engagementDates[].endDate | order(@ desc)[0],		"reviews": *[_type == "review" && references(^._id)] {			author,			title,			linkedIn,			"slug": slug.current,			body		}	} | order(latestEnd desc)
export type REVIEWS_BY_CLIENT_QUERY_RESULT = Array<{
  _id: string
  name: string | null
  relationship: 'Agency Partner' | 'Coworker' | 'Direct Client' | 'Student' | null
  role: string | null
  logo: {
    asset: SanityImageAssetReference | null
    crop: SanityImageCrop | null
...
// Source: ../web-next/src/sanity/queries/singletons.ts
// Variable: SINGLETON_INTRO_QUERY
// Query: *[_type == "singleton" && slug.current == $slug][0] {		_id,		title,		heroCopy,		"lede": pt::text(heroCopy),		bodyText	}
export type SINGLETON_INTRO_QUERY_RESULT = {
  _id: string
  title: string | null
  heroCopy: Array<{
    children?: Array<{
